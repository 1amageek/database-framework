...
struct PostgreSQLPointReadTests {
    ...
    func staticPointRead() async throws {
        try await PostgreSQLScenarioCoordinator.shared.withIsolatedScenario {
            ...
            let (fetched, missing) = try await context.withDataOperation {
                ...
                let missing = try await store.fetch(
                    PGPointReadItem.self,
                    id: uniqueID("missing")
                )
        ...
        try await PostgreSQLScenarioCoordinator.shared.withIsolatedScenario {
            ...
            try await context.withDataOperation {
                ...
                try await store.executeBatch(
                    inserts: [try PersistedModel(created)],
                    deletes: []
                )
    ...
    func partitionedPointRead() async throws {
        try await PostgreSQLScenarioCoordinator.shared.withIsolatedScenario {
            ...
            let (fetched, missing) = try await context.withDataOperation {
                ...
                let wrongStore = try await container.store(
                    for: TenantOrder.self,
                    path: wrongPath
                )
                let missing = try await wrongStore.fetch(
                    TenantOrder.self,
                    id: orderID
                )
                return (fetched, missing)
    ...
    func securedPointRead() async throws {
        try await PostgreSQLScenarioCoordinator.shared.withIsolatedScenario {
            ...
            let ownerContext = container.testBaseContext(authorization: owner)
