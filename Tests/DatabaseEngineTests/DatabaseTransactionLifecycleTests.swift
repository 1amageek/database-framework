...
struct DatabaseTransactionLifecycleTests {
    @Test("Nested transaction reuses the admitted container transaction")
    func nestedTransactionReusesAdmittedContainerTransaction() async throws {
        let container = try await makeContainer()
        defer { await container.shutdown() }
        let context = container.testBaseContext()

        try await context.withTransaction { outerTransaction in
            try await context.withTransaction { innerTransaction in
                #expect(innerTransaction === outerTransaction)
            }
        }
    }

    @Test("Commit validator reads after callback capability revocation")
    func commitValidatorUsesInternalReadCapability() async throws {
        let maintainer = FinalStateFetchingMaintainer()
        let container = try await makeContainer(maintainer: maintainer)
        defer { await container.shutdown() }
        let context = container.testBaseContext()
        let child = TransactionLifecycleChild(
            id: "validation-target",
            parentID: ""
        )

        try await context.withTransaction { transaction in
            try await transaction.save(child, precondition: .notExists)
        }
        try await context.withTransaction { transaction in
        ...
        #expect(
            try await context.model(
                for: "validation-owner",
                as: TransactionLifecycleParent.self
            )?.value == child.id
        )
    }

    @Test("Nested canonical read attenuates an outer write transaction")
    func nestedCanonicalReadAttenuatesOuterWriteTransaction() async throws {
        let container = try await makeContainer()
        defer { await container.shutdown() }
        let context = container.testBaseContext()

        _ = try await context.withTransaction { _ in
            try await context.executeCanonicalRead { _ in
                await #expect(
        ...
        let stored = try await context.model(
            for: "nested-read-write",
            as: TransactionLifecycleParent.self
        )
        #expect(stored == nil)
    }

    @Test("Nested read transaction closes at its callback boundary")
    func nestedReadTransactionClosesAtCallbackBoundary() async throws {
        let container = try await makeContainer()
        defer { await container.shutdown() }
        let context = container.testBaseContext()

        try await context.withTransaction { outerTransaction in
            let escaped = try await context.withPersistenceReadTransaction {
                nestedTransaction, _ in
                nestedTransaction
            }

            await #expect(throws: DatabaseTransactionError.closed) {
                try await escaped.fetch(
                    TransactionLifecycleParent.self,
                    identifiedBy: "escaped-nested-read"
                )
            }
            _ = try await outerTransaction.fetch(
                TransactionLifecycleParent.self,
                identifiedBy: ["outer-remains-open"]
            )
        }
    }

    @Test("Nested read transaction closes inside an outer raw read callback")
    func nestedReadTransactionClosesInsideRawReadCallback() async throws {
        let container = try await makeContainer()
        defer { await container.shutdown() }
        let context = container.testBaseContext()

        try await context.withReadStorageAccess { _ in
            let escaped = try await context.withPersistenceReadTransaction {
                nestedTransaction, _ in
                nestedTransaction
            }
            ...
            await #expect(throws: DatabaseTransactionError.closed) {
                try await escaped.fetch(
                    TransactionLifecycleParent.self,
                    identifiedBy: "escaped-nested-raw-read"
                )
            }
        }
    }

    @Test("Nested raw read capability closes at its callback boundary")
    func nestedRawReadCapabilityClosesAtCallbackBoundary() async throws {
        let container = try await makeContainer()
        defer { await container.shutdown() }
        let context = container.testBaseContext()

        try await context.withTransaction { _ in
            let key = try context.operationDataRoot().pack(
                Tuple("escaped-nested-storage-read")
            )
            let escaped = try await context.withReadStorageAccess {
                transaction in
                transaction
            }
            await #expect(
                ...
                _ = try await escaped.getValue(for: key, snapshot: true)
            }
        }
    }

    @Test("Nested storage access shares outer final validation")
    func nestedStorageAccessSharesOuterFinalValidation() async throws {
        let container = try await makeContainer(
            maintainer: FinalStateFetchingMaintainer()
        )
        defer { await container.shutdown() }
        let context = container.testBaseContext()

        await #expect(throws: FinalStateFetchingMaintainerError.self) {
            try await context.withTransaction { _ in
        ...
        #expect(
            try await context.model(
                for: "nested-final-validation",
                as: TransactionLifecycleParent.self
            ) == nil
        )
    }

    @Test("Nested transaction rejects a foreign database container")
    func nestedTransactionRejectsForeignContainer() async throws {
        let firstContainer = try await makeContainer()
        let secondContainer = try await makeContainer()
        defer {
            await firstContainer.shutdown()
            await secondContainer.shutdown()
        }
        let firstContext = firstContainer.testBaseContext()
        let secondContext = secondContainer.testBaseContext()

        try await firstContext.withTransaction { _ in
            await #expect(
                throws: DatabaseTransactionExecutionScopeError
                    .containerMismatch
            ) {
                try await secondContext.withTransaction { _ in () }
            }
            return ()
        }
    }
    ...
    @Test(
        "Inherited transaction context is revoked at callback return",
        .timeLimit(.minutes(1))
    )
    func inheritedTransactionContextIsRevoked() async throws {
        let container = try await makeContainer()
        defer { await container.shutdown() }
        let context = container.testBaseContext()
        let release = AsyncGate()

        let inheritedTask = try await context.executeCanonicalRead { _ in
            Task {
        ...
        await #expect(
            ...
            _ = try await inheritedTask.value
        }
    }

    #if DATABASE_MULTI_BASE
    @Test("Inherited database-root context is revoked at callback return")
    func inheritedDatabaseRootContextIsRevoked() async throws {
        let container = try await makeContainer()
        defer { await container.shutdown() }
        let release = AsyncGate()

        let inheritedTask = try await container.withExecutionDataRoot {
            Task {
                await release.waitUntilOpen()
    ...
    @Test(
        "Escaped write storage capability does not retain container shutdown",
        .timeLimit(.minutes(1))
    )
    func escapedWriteStorageCapabilityIsRevoked() async throws {
        let container = try await makeContainer()
        let context = container.testBaseContext()

        let escaped = try await context.withTransaction { transaction in
            (
                transaction,
                try context.operationDataRoot().subspace("data")
                    .pack(Tuple("escaped-write"))
            )
        }

        #expect(throws: DatabaseReadTransactionError.snapshotClosed) {
            try escaped.0.executionStorageAccess.setValue(
    ...
    @Test("Static directory binding clears a previously selected partition")
    func staticDirectoryBindingClearsStalePartition() async throws {
        let container = try await makeContainer()
        defer { await container.shutdown() }
        let context = container.testBaseContext()
        let partitioned = try IndexQueryContext(context: context)
            .withPartitions(
                try FieldObject([
        ...
        #expect(try partitioned.partitionBinding(for: TenantOrder.self) != nil)

        let unpartitioned = try partitioned.withPartitions(
            FieldObject(),
            for: TransactionLifecycleParent.self
        )

        #expect(
            try unpartitioned.partitionBinding(
                for: TransactionLifecycleParent.self
            ) == nil
        )
        #expect(unpartitioned.partitionValues.fields.isEmpty)
    }
...
private struct FinalStateFetchingMaintainer: PersistableMutationMaintainer {
    let identifier = "test.transaction.final-state-fetch"
    ...
    func validateFinalState(
        ...
        for model in models
        where model.entity == TransactionLifecycleParent.persistableType {
            guard case .string(let targetID)? = model.value(
                forFieldNamed: "value"
            ) else {
                throw FinalStateFetchingMaintainerError.invalidTarget
            }
            let identity = try EntityReference(
                entity: TransactionLifecycleChild.persistableType,
                id: .string(targetID)
            )
            guard try await context.fetch(identity) != nil else {
                throw FinalStateFetchingMaintainerError.missingTarget
            }
        }
    }
}

private enum FinalStateFetchingMaintainerError: Error {
    case invalidTarget
    case missingTarget
}
