...
struct DirectoryLayerTagMapTests {
    ...
    func derivesPartitionForEveryDeclarationPassingThrough() throws {
        let map = try DirectoryLayerTagMap(entities: [try Self.tenant(), try Self.order()])
    ...
    func derivationIsIndependentOfDeclarationOrder() throws {
        let forward = try DirectoryLayerTagMap(entities: [try Self.tenant(), try Self.order()])
        let reverse = try DirectoryLayerTagMap(entities: [try Self.order(), try Self.tenant()])
    ...
    func positionWithoutALeafIsAPlainDirectory() throws {
        let map = try DirectoryLayerTagMap(entities: [try Self.order()])
    ...
    func declarationWithoutComponentsOccupiesNoPosition() throws {
        ...
        let map = try DirectoryLayerTagMap(entities: [bare])
    ...
    func rejectsLeafLayerDisagreement() throws {
        ...
        #expect(throws: expected) { try DirectoryLayerTagMap(entities: [alpha, beta]) }
        #expect(throws: expected) { try DirectoryLayerTagMap(entities: [beta, alpha]) }
    }
    ...
    func rejectsFieldKindWithoutACanonicalComponent() throws {
        let vector = try Self.entity(
            "Embedding",
            fields: [Self.field("point", 1, .vector)],
            components: [.staticPath("v"), .dynamicField(fieldName: "point")]
        )
        #expect(
            throws: DirectoryLayerTagError.unsupportedDynamicFieldKind(
                entity: "Embedding", fieldName: "point", kind: .vector
            )
        ) { try DirectoryLayerTagMap(entities: [vector]) }

        let term = try Self.entity(
            "Statement",
        ...
        #expect(
            throws: DirectoryLayerTagError.unsupportedDynamicFieldKind(
                entity: "Statement", fieldName: "subject", kind: .rdfTerm
            )
        ) { try DirectoryLayerTagMap(entities: [term]) }

        let reference = try Self.entity(
            "Membership",
        ...
        #expect(
            throws: DirectoryLayerTagError.unsupportedDynamicFieldKind(
                entity: "Membership", fieldName: "owner", kind: .reference
            )
        ) { try DirectoryLayerTagMap(entities: [reference]) }
    }
    ...
    func rejectsStaticComponentInTheCanonicalImage() throws {
        ...
        #expect(
            throws: DirectoryLayerTagError.staticComponentInCanonicalImage(
                entity: "Shadow", component: "s-abc"
            )
        ) { try DirectoryLayerTagMap(entities: [shadowing]) }
        ...
        let map = try DirectoryLayerTagMap(entities: [ordinary])
    ...
    func admitsSharedEnumPositionAcrossRawRepresentations() throws {
        ...
        let map = try DirectoryLayerTagMap(entities: [alpha, beta])
