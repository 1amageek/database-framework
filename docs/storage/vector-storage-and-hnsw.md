...
| 2026-08-12 | Final responsibility-split source: standard WASI, Embedded WASM, and static Musl Linux `Database` and `DatabaseWireAdapter` products with `AllRuntimeFeatures,MultipleBases`; the adapter build transitively compiles `DatabaseOperations` | Passed: all six non-redundant product/platform builds compiled and linked with the pinned 2026-07-23 Swift 6.4 toolchain and matching SDKs |
