...
public struct ScalarIndexSearcher: Sendable {
    ...
    /// Visits matching entries one at a time without materializing an
    /// unbounded result array. Returning `false` stops the scan.
    package func forEach(
        query: ScalarIndexQuery,
        in subspace: Subspace,
        using reader: StorageReader,
        _ visit: (borrowing IndexEntry) throws -> Bool
    ) async throws {
        if let limit = query.limit {
            guard limit >= 0 else {
                throw ScalarIndexSearchError.invalidLimit(limit)
        ...
        let cursorSubspace: Subspace
        let start: Tuple?
        let end: Tuple?
        let startInclusive: Bool
        let endInclusive: Bool
        if let queryStart = query.start,
           let queryEnd = query.end,
           Tuple(queryStart).pack() == Tuple(queryEnd).pack() {
            guard query.startInclusive && query.endInclusive else { return }
            cursorSubspace = Subspace(
                prefix: subspace.prefix.appending(
                    contentsOf: Tuple(queryStart).pack()
                )
            )
            start = nil
            end = nil
            ...
            cursorSubspace = subspace
            start = query.start.map(Tuple.init)
            end = query.end.map(Tuple.init)
            startInclusive = query.startInclusive
            endInclusive = query.endInclusive
        }

        var cursor = try reader.rangeCursor(
            subspace: cursorSubspace,
            start: start,
            end: end,
        ...
        do {
            while let (key, value) = try await cursor.next() {
                ...
                visited += 1
                guard try visit(entry),
                      query.limit.map({ visited < $0 }) ?? true else {
                    try await cursor.finish()
                    return
                }
            }
