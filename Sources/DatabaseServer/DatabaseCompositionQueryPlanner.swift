...
struct DatabaseCompositionQueryPlanner {
    ...
    private struct MergeHeap {
        private(set) var rows: [(memberIndex: Int, row: OriginRow)] = []
        ...
        mutating func removeFirst(
            orderedBefore: (OriginRow, OriginRow) throws -> Bool
        ) throws -> (memberIndex: Int, row: OriginRow)? {
    ...
    func execute(
        ...
        snapshotStore: DatabaseCompositionSnapshotStore?
    ...
    private func executeRows(
        ...
        guard memberCount <= maximumBufferedRows else {
            throw DatabaseWorkLimitError.maximumIntermediateRows(
                stage: .resultMaterialization,
                consumed: 0,
                requested: UInt64(memberCount),
        ...
        let localPageSize = max(
            ...
            min(
                requestedPageLimit,
                maximumBufferedRows / memberCount
            )
        )
        ...
        if let mergeOrdering {
            ...
            for index in cursors.indices {
                if let row = try await nextRow(
                    ...
                    try heap.insert(
                        memberIndex: index,
                        row: prepared(row, member: cursors[index].member)
                    ) { lhs, rhs in
                        try compare(
                            lhs,
                            rhs,
            ...
            while let head = try heap.removeFirst(orderedBefore: {
                lhs,
                rhs in
                try compare(
                    lhs,
                ...
                if let row = try await nextRow(
                    cursor: &cursors[head.memberIndex],
                    query: localQuery,
                    pageSize: localPageSize,
                    request: request,
                    source: source,
                    ...
                    try heap.insert(
                        ...
                        row: prepared(
                            row,
                            member: cursors[head.memberIndex].member
                        )
                        ) { lhs, rhs in
                            try compare(
                                lhs,
    ...
    private func nextRow(
        ...
        while cursor.nextRowIndex >= cursor.rows.count {
            ...
            cursor.rows = response.rows
            cursor.nextRowIndex = 0
    ...
    private func rowFootprint(
        ...
        for (key, value) in row.fields.sorted(by: { $0.key < $1.key }) {
            ...
            bytes = try Self.adding(
                bytes,
                UInt64(
                    try FieldValueTupleCodec.prepareComposite(
                        value,
                        limits: .default
                    ).encodedByteCount
                )
