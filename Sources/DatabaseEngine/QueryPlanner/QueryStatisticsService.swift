...
/// Collects, stores, and serves query-planning statistics.
///
...
/// let planner = QueryPlanner(statistics: statistics)
/// ```
///
/// **Thread Safety**:
...
public final class QueryStatisticsService: StatisticsProvider, Sendable {
    ...
    public func equalitySelectivity<T: Persistable>(
        ...
        let comparableValue: FieldValue
        do {
            comparableValue = try TypeConversion.toFieldValue(value)
        } catch {
            return nil
        }
    ...
    public func rangeSelectivity<T: Persistable>(field: String, range: RangeBound, type: T.Type) -> Double? {
        ...
        guard let (minValue, maxValue, minInclusive, maxInclusive) =
            convertRangeBound(range) else {
            return nil
        }
    ...
    public func inSelectivity<T: Persistable>(
        ...
        var comparableValues: [FieldValue] = []
        comparableValues.reserveCapacity(values.count)
        for value in values {
            do {
                comparableValues.append(try TypeConversion.toFieldValue(value))
            } catch {
                return nil
            }
        }
        ...
        for item in items {
            ...
            for field in fieldsToCollect {
                if let value = item[dynamicMember: field] {
                    ...
                    do {
                        fieldValue = try TypeConversion.toFieldValue(value)
                    } catch let error {
                        throw StatisticsCollectionError.invalidRuntimeValue(
                            field: field,
                            reason: error
                        )
                    }
    ...
    private func convertRangeBound(_ range: RangeBound) -> (
        ...
        if let lower = range.lower {
            do {
                minValue = try TypeConversion.toFieldValue(lower.value)
            } catch {
                return nil
            }
        ...
        if let upper = range.upper {
            do {
                maxValue = try TypeConversion.toFieldValue(upper.value)
            } catch {
                return nil
            }
