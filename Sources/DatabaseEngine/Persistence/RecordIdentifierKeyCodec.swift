...
public enum RecordIdentifierKeyCodec {
    ...
    public static func tuple(
        ...
        try tuple(
            for: model.recordIdentifierValue,
            expectedType: type(of: model).recordIdentifierType,
            limits: limits
        )
    }
    ...
    public static func tuple(
        for value: RecordIdentifierValue,
        expectedType: RecordIdentifierType,
        limits: RecordIdentifierLimits = .default
    ) throws(RecordIdentifierValidationError) -> Tuple {
        try validate(
        ...
        return Tuple(RecordIdentifierTupleElement(value: value))
    }
    ...
    public static func value(
        ...
        expectedType: RecordIdentifierType,
        limits: RecordIdentifierLimits = .default
    ) throws(RecordIdentifierKeyError) -> RecordIdentifierValue {
        do {
            try RecordIdentifierValidator.validate(expectedType, limits: limits)
        } catch let error {
    ...
    private static func decodeNode(
        ...
        if let canonical = element as? RecordIdentifierTupleElement {
            do {
                try validate(
                    canonical.value,
                    expectedType: expectedType,
        ...
        case .composite(let expectedComponents):
            ...
            var components: [RecordIdentifierValue] = []
