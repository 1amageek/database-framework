...
///
/// Admission happens before the backend await and reserves the issued maximum
/// against both retained and other pending point reads. On success the
/// allowance is transferred to exact retained ownership; all other paths
...
package func readPointValue(
    ...
    do {
        value = try await transaction.getValue(
            for: key,
            snapshot: snapshot,
