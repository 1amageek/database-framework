...
extension RelationshipIndexKind: IndexKindMaintainable {
    ...
    public func makeIndexMaintainer<Item: Persistable>(
        ...
        guard let config = relationshipConfig else {
            fatalError(
                "RelationshipIndexConfiguration not found for index '\(index.name)' on type '\(Item.persistableType)'. " +
                "Ensure FDBContainer is properly initialized with the schema containing this type."
            )
        }
